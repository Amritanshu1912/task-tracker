        ...
        {isSidebarOpen && (
          <SidebarSection title="Filters" icon={FilterIcon}>
            <div className="flex items-center justify-between h-7 mb-1.5">
              <span className="text-xs font-medium text-muted-foreground pl-0">
              ...
              {activeLabelFilters.length > 0 && (
                <Button
                  ...
                  className="h-6 w-6 text-destructive hover:text-destructive" // Adjusted size for icon button
                  title="Clear label filters" // Added title for accessibility
                >
                  <Trash2 className="h-3.5 w-3.5" /> {/* Trash icon */}
                </Button>
              )}
            </div>
            <div className="flex flex-wrap gap-1.5 mb-4">
              {AVAILABLE_LABELS.map((label) => {
                ...
                return (
                  ...
                  >
                    <span className="mr-0.5">{LABEL_EMOJIS[label]}</span>{" "}
                    {label}
                  </Badge>
                );
              })}
